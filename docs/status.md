...
1. **【ホスト】#19 の merge**（`drizzle/` を含むので人間 merge、auto-merge は arm していない）: `SESSION_SECRET` を put（merge 前後どちらでも。[plans/host-setup.md](plans/host-setup.md) 6）→ Deploy 後に `/health` と `register/begin` = `registration_closed` を確認。okayus-skills の還元（passkey-auth 0.2.0 / space-membership-invite 0.2.0 / e2e-playwright 0.2.1）を `cd ../okayus-skills` で commit
...
- #19 は人間 merge 待ち。`SESSION_SECRET` の投入もホスト作業（backup は今回不要 — データなし・追加のみ）
