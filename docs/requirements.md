...
2026-08-22 時点の要件（3〜5・8 の改訂と 10 の追加は 2026-09-03 — [ADR-007](adr/007-recipe-links-ogp.md)、11〜13 の追加は 2026-09-03、11 の詳細は [ADR-008](adr/008-meal-edit.md)、14〜15 の追加は 2026-09-05）。2026-05 の製品意図（CLAUDE.md「このアプリは何のためにあるか」）を前提に、機能要件を列挙する。変更はこのファイルを更新してから実装する。
...
15. **写真ファーストのトップページ** — トップページは検索と投稿済み写真を主役にする。タイムラインの既定は「写真だけ」（「くわしく」へは切り替え）
