...
- **新しい注入点**: `./up.sh` = 素の `docker compose up -d`（**資格情報ゼロ・冪等**）。token は **`./shell.sh`**（= `op read で op:// 参照を解決 → docker exec -e GH_TOKEN matatabetai-dev …`）が開いたシェルとその子（claude / git / gh）にだけ入る。`docker exec -e NAME`（値なし）はホストプロセスの env から転送し、未設定なら何も渡さない＝ fail closed のまま。値は argv に載らないので `ps` に出ない。
